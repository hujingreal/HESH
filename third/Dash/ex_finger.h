      ...
      for (int j = 0; j < kNumPairPerBucket; ++j) {
        if (CHECK_BIT(mask, j)) {
          if constexpr (std::is_pointer_v<T>) {
            ...
            key_hash = hash_func(curr_key->key, curr_key->length);
          } else {
            key_hash = hash_func(curr_bucket->_[j].key, sizeof(size_t));
          }
...
void Finger_EH<T>::Halve_Directory() {
  std::cout << "Begin::Directory_Halving towards " << dir->global_depth
            << std::endl;
  auto d = dir->_;
  ...
  dir = new_dir;
#endif
  std::cout << "End::Directory_Halving towards " << dir->global_depth
            << std::endl;
}
...
int Finger_EH<T>::Insert(T key, Value_t value) {
  ...
  if constexpr (std::is_pointer_v<T>) {
    ...
    key_hash = hash_func(key, sizeof(key));
  }
...
int Finger_EH<T>::FindAnyway(T key) {
  ...
  if constexpr (std::is_pointer_v<T>) {
    ...
    key_hash = hash_func(key->key, key->length);
  } else {
    key_hash = hash_func(key, sizeof(key));
  }
